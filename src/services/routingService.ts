...
class RoutingService {
  ...
  private static generateFallbackRoute(p: [number, number], d: [number, number]): any {
    ...
    for (let i = 0; i <= points; i++) {
      ...
      const lat = p[0] + (d[0] - p[0]) * ratio;
      const lng = p[1] + (d[1] - p[1]) * ratio;
      
      // Adiciona um pequeno "ruído" para não ser uma linha perfeitamente reta
      const jitter = i === 0 || i === points ? 0 : (Math.random() - 0.5) * 0.0015;
      coords.push([lng + jitter, lat + jitter]);
    }
