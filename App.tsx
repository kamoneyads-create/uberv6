...
import { generateTripRequest, preloadCityData, getCityCoords, generateInstantMockTrip } from './services/geminiService';
...
const SplashScreen: React.FC<{ onComplete: () => void }> = ({ onComplete }) => {
  ...
  return (
    <div className={`absolute inset-0 z-[10000] bg-black flex flex-col items-center justify-center pb-32 transition-transform duration-700 ease-in-out ${screenSlide ? '-translate-x-full' : 'translate-x-0'}`}>
...
const App: React.FC = () => {
  ...
  const [userData, setUserData] = useState(() => {
    ...
    if (currentId) {
      ...
      if (saved) {
        ...
        if (parsed.bankAccount === undefined) parsed.bankAccount = 'N/A';
  ...
  useEffect(() => {
    const option = userData.darkModeOption || DarkModeOption.ALWAYS_OFF;
    if (option === DarkModeOption.ALWAYS_ON) {
      setIsDarkMode(true);
    } else if (option === DarkModeOption.ALWAYS_OFF) {
      setIsDarkMode(false);
    } else if (option === DarkModeOption.USE_PHONE_SETTING) {
    ...
    } else if (option === DarkModeOption.AUTOMATIC) {
      const checkTime = () => {
  ...
  useEffect(() => {
    const fastPreload = async () => {
      try {
        ...
        const tripType = parsed.tripType || 'UberX';
        
        if (city) {
          console.log(`[🚀 Fast Preload] Iniciando para ${city} durante Splash Screen...`);
          // Dispara em paralelo para ganhar tempo
          preloadCityData(city);
          getCityCoords(city).then(coords => {
            if (coords) {
              generateTripRequest(city, tripType, 0, coords);
            }
          });
        }
      } catch (e) {
        console.error("[Fast Preload] Erro:", e);
      }
    };
  ...
  useEffect(() => {
    if (userData.city) {
      // Limpa a viagem anterior ao mudar de cidade para evitar endereços da cidade antiga
      setNextTrip(null);
      nextTripRef.current = null;
      
      preloadCityData(userData.city);
      getCityCoords(userData.city).then(coords => {
        setCurrentCoords(coords);
        // Pré-carrega uma viagem imediatamente para o cache com as coordenadas
        generateTripRequest(userData.city, userData.tripType, lastTripIndexRef.current, coords);
      });
    }
  }, [userData.city]);
  ...
  const persistUserData = (newData: any, identifier?: string) => {
    ...
    // REMOVIDO: Não revogar o vídeo aqui, pois causa o erro de vídeo em branco
    // ao salvar qualquer outra informação do perfil.
    // O vídeo deve persistir enquanto a sessão for a mesma.
  ...
  const preloadVerificationVideo = async (force = false) => {
    ...
    try {
      ...
      console.log('Iniciando preload do vídeo de verificação (Base64)...');
      const videoUrl = await getVideo();
      if (videoUrl && videoUrl.length > 100) {
        setPreloadedVideoUrl(videoUrl);
        console.log('Vídeo de verificação pré-carregado com sucesso (Base64)');
      } else {
        console.log('Nenhum vídeo encontrado para preload');
  ...
  const nextTripRef = useRef<TripRequest | null>(null);

  useEffect(() => {
    nextTripRef.current = nextTrip;
  }, [nextTrip]);

  // Efeito para pré-carregar a próxima viagem em background de forma agressiva
  // Agora começa assim que a cidade estiver disponível, mesmo antes do login completo
  useEffect(() => {
    if (nextTrip || newRequest || acceptedTrip || !userData.city) return;

    let isMounted = true;
    const preload = async () => {
      if (isFetchingRef.current) return;
      isFetchingRef.current = true;
      console.log("Pre-loading next trip aggressively (App Start/City Change)...");
      try {
        // Tenta obter coordenadas se ainda não tiver
        const coords = currentCoords || await getCityCoords(userData.city);
        if (!currentCoords && isMounted && coords && !isNaN(coords[0]) && !isNaN(coords[1])) {
          setCurrentCoords(coords);
        }

        const generated = await generateTripRequest(userData.city, userData.tripType, lastTripIndexRef.current, coords);
        if (isMounted) {
          setNextTrip(generated as unknown as TripRequest);
          console.log("Next trip pre-loaded and ready with route.");
        }
      } catch (e) {
        console.error("Erro no pre-load agressivo:", e);
      } finally {
        isFetchingRef.current = false;
      }
    };

    // Inicia o pre-load o mais rápido possível
    const timer = setTimeout(preload, 100);
    return () => { 
      isMounted = false; 
      clearTimeout(timer);
    };
  }, [nextTrip === null, newRequest === null, acceptedTrip === null, userData.city, userData.tripType, !!currentCoords]);

  // NOVO: Pre-load imediato ao ficar online se ainda não tiver viagem
  useEffect(() => {
    if (isOnline && !nextTrip && !newRequest && !acceptedTrip && userData.city) {
      console.log("Usuário ficou online sem viagem pré-carregada. Iniciando busca imediata...");
      const coords = currentCoords || undefined;
      isFetchingRef.current = true;
      generateTripRequest(userData.city, userData.tripType, lastTripIndexRef.current, coords)
        .then(generated => {
          if (!nextTripRef.current) setNextTrip(generated as unknown as TripRequest);
        })
        .finally(() => {
          isFetchingRef.current = false;
        });
  ...
  useEffect(() => {
    ...
    if (!dispatchStartTimeRef.current) {
      dispatchStartTimeRef.current = Date.now();
      console.log("Iniciando contagem regressiva de 14s para despacho...");
    }

    const timer = setInterval(async () => {
      const elapsed = Date.now() - dispatchStartTimeRef.current!;
      
      if (dispatchTrigger && !newRequest && !acceptedTrip) {
        const currentNextTrip = nextTripRef.current;
        
        // Se já passaram 14 segundos
        if (elapsed >= 14000) {
          clearInterval(timer);
          dispatchStartTimeRef.current = null;
          
          if (currentNextTrip) {
            console.log("Despachando viagem pré-carregada aos 14s!");
            setNewRequest(currentNextTrip);
            lastRequestIdsRef.current = [currentNextTrip.id, ...lastRequestIdsRef.current.slice(0, 10)];
            lastTripIndexRef.current = (currentNextTrip as any).index;
            setNextTrip(null);
          } else {
            // Se ainda não carregou aos 14s, força um mock instantâneo para não atrasar
            console.log("CRÍTICO: Viagem não pronta aos 14s, gerando mock instantâneo...");
            generateInstantMockTrip(userData.city).then(generated => {
        ...
        // Se passaram 10 segundos e ainda não temos a viagem, começa a buscar agora
        else if (elapsed >= 10000 && !currentNextTrip && !isFetchingRef.current) {
          console.log("Aviso: Viagem não pré-carregada aos 10s, iniciando busca emergencial antecipada...");
          isFetchingRef.current = true;
          generateTripRequest(userData.city, userData.tripType, lastTripIndexRef.current, currentCoords || undefined)
            .then(generated => {
              if (!nextTripRef.current) setNextTrip(generated as unknown as TripRequest);
            })
            .finally(() => {
