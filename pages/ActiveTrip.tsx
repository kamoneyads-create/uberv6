        ...
        const fetchRoute = async () => {
          ...
          try {
            if (!routeData && !cancelled) {
              ...
              console.log("Buscando rota OSRM (fallback):", url);
              ...
              if (response.ok && !cancelled) {
                ...
                if (contentType && contentType.includes("application/json")) {
                  ...
                  if (routeData.routes?.[0]?.geometry) {
                    setCachedRoute(routeKey, routeData.routes[0].geometry);
                  }
                } else {
                  const text = await response.text();
                  console.error("OSRM response not JSON:", text.substring(0, 100));
                }
              } else if (!cancelled) {
                console.error("Falha ao buscar rota OSRM:", response.status);
              }
            }
          } catch (fetchErr) {
