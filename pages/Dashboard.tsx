...
import { getCityCoords, generateMockRoute } from '../services/geminiService';
...
const TrendsChartIcon = ({ size = 32 }: { size?: number }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M3 3V21H21V18H6V3H3Z" fill="currentColor" />
    <rect x="8" y="10" width="3" height="8" fill="currentColor" />
    <rect x="13" y="13" width="3" height="5" fill="currentColor" />
...
const TargetLocationIcon = ({ size = 32 }: { size?: number }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    <circle cx="12" cy="12" r="6.5" stroke="currentColor" strokeWidth="2.5" />
    <circle cx="12" cy="12" r="2" fill="currentColor" />
    <path d="M12 2V5" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" />
    <path d="M12 19V22" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" />
    <path d="M2 12H5" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" />
    <path d="M19 12H22" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" />
  </svg>
);

const SettingsIcon = () => (
  <svg width="32" height="32" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M4 8H20" stroke="currentColor" strokeWidth="3.5" strokeLinecap="round"/>
    <path d="M4 16H20" stroke="currentColor" strokeWidth="3.5" strokeLinecap="round"/>
    <circle cx="8" cy="8" r="4" fill="currentColor" stroke="white" strokeWidth="1.5" />
    <circle cx="16" cy="16" r="4" fill="currentColor" stroke="white" strokeWidth="1.5" />
  </svg>
);

const ListIcon = () => (
  <svg width="32" height="32" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M9 6H20" stroke="currentColor" strokeWidth="3.5" strokeLinecap="round"/>
    <path d="M9 12H20" stroke="currentColor" strokeWidth="3.5" strokeLinecap="round"/>
    <path d="M9 18H20" stroke="currentColor" strokeWidth="3.5" strokeLinecap="round"/>
...
const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  const totalTime = 14;
  const ZOOM_LEVEL_ONLINE = 14.8; // Reduzido conforme solicitado
        ...
        markerRef.current = L.marker(currentCoords, {
          ...
          icon: L.divIcon({
            ...
            html: `<div class="relative w-12 h-12 flex items-center justify-center">
                    <div class="absolute w-full h-full bg-[#276EF1] rounded-full animate-gps-pulse opacity-0"></div>
                    <div class="absolute inset-0 bg-white border-black rounded-full border-[5px] shadow-lg z-10"></div>
  ...
  // Efeito para atualizar o tema do mapa dinamicamente (Removido logicamente)
  useEffect(() => {
  ...
  useEffect(() => {
    if (currentView === AppView.DASHBOARD && mapRef.current && currentCoords && mapReady && !showSplash && !isNaN(currentCoords[0]) && !isNaN(currentCoords[1])) {
      ...
      if (lastViewRef.current !== AppView.DASHBOARD || isFirstLoadRef.current) {
        ...
        const startZoom = targetZoom - 2.5; 
        mapRef.current.setView(currentCoords, startZoom, { animate: false });
  ...
  useEffect(() => {
    ...
    if (userData.dynamicOnMap === 'Não') return;
    ...
    hotSpots.forEach(spot => {
      ...
      if (!(spot as any).hidePill && userData.dynamicOnMap === 'Sim') {
  ...
  }, [currentCoords, mapReady, userData.dynamicOnMap]);

  // Lógica para desenhar a rota e marcadores de viagem no mapa
  useEffect(() => {
    if (!mapRef.current || !mapReady || !leafletLoaded) return;
    ...
    if (activeRequest) {
      // 1. Desenhar a Polilinha da Rota com Casing (Mesmo estilo do ActiveTrip)
      if (activeRequest.routeGeometry) {
        const coords = activeRequest.routeGeometry.coordinates.map((c: any) => [c[1], c[0]]);
        
        // Casing (Sombra/Borda da linha)
        const routeCasing = L.polyline(coords, {
          color: '#1a4bb3',
          weight: 10,
          opacity: 0.4,
          lineCap: 'round',
        ...
        // Main Line
        const routeMain = L.polyline(coords, {
          color: '#276EF1',
          weight: 6,
          opacity: 1,
          lineCap: 'round',
        ...
        routeLayerRef.current = L.layerGroup([routeCasing, routeMain]).addTo(mapRef.current);
      } else if (activeRequest.pickupCoords && activeRequest.destinationCoords) {
        // Fallback: Rota mockada "curvy" se não houver geometria real
        const mockRoute = generateMockRoute(activeRequest.pickupCoords, activeRequest.destinationCoords);
        const coords = mockRoute.coordinates.map((c: any) => [c[1], c[0]]);
      ...
      // 2. Adicionar Marcadores de Pickup e Destino
      if (activeRequest.pickupCoords) {
        pickupMarkerRef.current = L.marker(activeRequest.pickupCoords, {
          icon: L.divIcon({
            className: 'uber-pickup-marker',
            html: `<div class="w-4 h-4 bg-black border-2 border-white rounded-full shadow-lg"></div>`,
            iconSize: [16, 16],
            iconAnchor: [8, 8]
          }),
          interactive: false
      ...
      if (activeRequest.destinationCoords) {
        destMarkerRef.current = L.marker(activeRequest.destinationCoords, {
          icon: L.divIcon({
            className: 'uber-dest-marker',
            html: `<div class="w-4 h-4 bg-black border-2 border-white shadow-lg"></div>`,
            iconSize: [16, 16],
            iconAnchor: [8, 8]
          }),
          interactive: false
      ...
      // 3. Ajustar o zoom para ver a rota toda (Restaurado para 1.5s)
      if (activeRequest.pickupCoords && activeRequest.destinationCoords) {
        const bounds = L.latLngBounds([
          activeRequest.pickupCoords, 
          activeRequest.destinationCoords,
          currentCoords || activeRequest.pickupCoords // Inclui o motorista no zoom
        ]);
      ...
      // Se não houver oferta ativa, volta para o zoom do motorista
      if (isOnline && currentCoords && mapReady) {
        mapRef.current.flyTo(currentCoords, ZOOM_LEVEL_ONLINE, { duration: 1.5 });
      }
    }
  }, [activeRequest, mapReady, leafletLoaded]);
  ...
  return (
    ...
    >
          ...
          <motion.div variants={itemVariants} className="pt-[calc(env(safe-area-inset-top)+1rem)] px-6 pb-6 bg-white flex-shrink-0 z-10">
            <div className="flex justify-between items-start">
              ...
              <div className="flex flex-col gap-3 items-end pt-1">
                <button className="w-12 h-12 bg-white force-white-bg rounded-full flex items-center justify-center shadow-sm active:scale-90 transition-transform"><i className="fa-solid fa-shield-halved text-[#276EF1] text-xl"></i></button>
                <button className="w-12 h-12 bg-white force-white-bg rounded-full flex items-center justify-center shadow-sm active:scale-90 transition-transform"><i className="fa-solid fa-sliders text-black force-black text-xl"></i></button>
              </div>
            </div>
          </motion.div>
        )}
        ...
        <motion.div variants={itemVariants} className={`overflow-hidden shrink-0 ${isOnline ? 'absolute inset-0 z-10 h-full w-full' : 'relative mx-4 mb-6 rounded-[32px] shadow-sm border border-gray-100 h-[365px]'}`}>
          ...
          {isOnline && (
            <div className="absolute inset-0 pointer-events-none flex flex-col z-30">
              ...
              >
              <button className="w-14 h-14 bg-white force-white-bg rounded-full shadow-[0_4px_15px_rgba(0,0,0,0.15)] flex items-center justify-center pointer-events-auto active:scale-90 transition-transform">
                <BlueShieldIcon size={34} />
              </button>
            </motion.div>

              <div className={`absolute bottom-[100px] right-6 z-40 flex flex-col gap-3.5 pointer-events-none transition-all duration-300 ${isDrawerOpen ? 'opacity-0 translate-y-4' : 'opacity-100 translate-y-0'}`}>
                <motion.div 
                  initial={{ x: 60, opacity: 0 }}
                  animate={{ x: 0, opacity: 1 }}
                ...
                >
                  <button 
                    ...
                    className="w-14 h-14 bg-white force-white-bg rounded-full shadow-[0_4px_15px_rgba(0,0,0,0.15)] flex items-center justify-center pointer-events-auto active:scale-90 transition-transform"
                  >
                   ...
                   <div className="flex items-center justify-between w-full mb-1">
                      ...
                      <div className="flex flex-col items-center flex-1 px-2">
                        ...
                        {isOnline && !activeRequest && (
                          <div className="h-1.5 mt-0.5"></div>
                        )}
                      </div>
                      <div className={`transition-opacity duration-300 ${isDrawerOpen ? 'opacity-20' : 'opacity-100'}`}>
        ...
        {!isOnline && (
          <motion.div variants={itemVariants} className="px-6 pb-40">
            ...
            >
              ...
              <div className="w-11 h-11 bg-white force-white-bg rounded-full flex items-center justify-center"><i className="fa-solid fa-arrow-right text-black force-black"></i></div>
            </div>
            ...
            <h3 className="text-[28px] font-black text-black leading-[34px] mb-3 tracking-tighter">Os ganhos das viagens são altos em {userCity.split(',')[0]}</h3>
      ...
      <AnimatePresence>
        {(!isOnline || isConnecting) && !isOnline && currentView === AppView.DASHBOARD && !showSplash && (
          <motion.div 
            ...
            <div className="flex gap-4 mb-5">
              ...
              <div className="flex flex-col gap-4 flex-1">
                <div className="flex flex-col">
                  ...
                  <p className="text-[14px] font-medium text-[#545454] leading-snug">
                    {AddressManager.formatAddress(activeRequest.pickup)}
                  </p>
                </div>
                <div className="flex flex-col">
                  <p className="text-[16px] font-bold text-black leading-tight">
                  ...
                  <p className="text-[14px] font-medium text-[#545454] leading-snug">
                    {AddressManager.formatFullAddress(activeRequest.destination)}
                  </p>
                </div>
              </div>
            </div>
            
            {activeRequest.groundingLinks && activeRequest.groundingLinks.length > 0 && (
              <div className="mb-4 px-1">
                <p className="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-2">Fontes Verificadas</p>
                <div className="flex flex-wrap gap-2">
                  {activeRequest.groundingLinks.slice(0, 3).map((link: any, idx: number) => (
                    <a 
                      key={idx} 
                      href={link.uri} 
                      target="_blank" 
                      rel="noopener noreferrer"
                      className="text-[10px] bg-gray-50 text-blue-600 px-2 py-1 rounded-md border border-gray-100 flex items-center gap-1 active:scale-95 transition-transform"
                    >
                      <i className="fa-solid fa-location-dot text-[8px]"></i>
                      {link.title && link.title.length > 15 ? link.title.substring(0, 15) + '...' : (link.title || "Ver no Mapa")}
                    </a>
                  ))}
                </div>
              </div>
