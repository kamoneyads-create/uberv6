...
const EditProfile: React.FC<EditProfileProps> = ({ 
  ...
  useEffect(() => {
    ...
    const checkVideo = async () => {
      try {
        ...
        console.log(`[EditProfile] Verificando vídeo no banco para sessão: ${editingSessionId}...`);
        const videoData = await getVideo(editingSessionId);
        
        if (!active) return;

        if (videoData && videoData.length > 100) {
          console.log(`[EditProfile] Vídeo encontrado no banco (${videoData.length} chars)`);
          setHasVideo(true);
          setVideoPreviewUrl(videoData);
        } else {
          console.log('[EditProfile] Nenhum vídeo encontrado no banco');
          setHasVideo(false);
          setVideoPreviewUrl(null);
        }
      } catch (err) {
        console.error('[EditProfile] Erro ao verificar vídeo:', err);
      } finally {
    ...
    return () => {
      active = false;
      // Não é mais necessário revogar URLs Base64
    };
  }, [retryCount]);
  ...
  const flipImageHorizontal = (base64Str: string): Promise<string> => {
    return new Promise((resolve) => {
      const img = new Image();
      img.src = base64Str;
      img.onload = () => {
  ...
  const handleVehicleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = async () => {
        ...
        // Inverter horizontalmente automaticamente conforme solicitado
        const flipped = await flipImageHorizontal(compressed);
        setFormData(prev => ({ ...prev, vehicleImage: flipped }));
  ...
  const handleVideoFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      // Criar preview temporário para feedback imediato
      const reader = new FileReader();
      reader.onloadend = () => {
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    console.log('[EditProfile] Botão Salvar clicado. Verificando vídeo...', { hasVideoBlob: !!videoBlob, hasVideoState: hasVideo });
    
    try {
      if (videoBlob) {
        try {
          console.log(`[EditProfile] Tentando salvar vídeo para ${editingSessionId}. Tamanho: ${videoBlob.size}`);
          await saveVideo(videoBlob, editingSessionId);
          console.log('[EditProfile] Vídeo salvo no IndexedDB com sucesso');
          
          // Após salvar, limpar o blob local e forçar o recarregamento do vídeo do banco
          setVideoBlob(null);
          setRetryCount(prev => prev + 1);
        } catch (videoError) {
          console.error('[EditProfile] Erro ao salvar vídeo no IndexedDB:', videoError);
          alert('Erro ao salvar vídeo: ' + (videoError instanceof Error ? videoError.message : 'Erro desconhecido'));
        }
      } else if (!hasVideo) {
        console.log(`[EditProfile] Removendo vídeo do IndexedDB para ${editingSessionId}...`);
        await deleteVideo(editingSessionId);
      }
      ...
      console.error('[EditProfile] Erro ao salvar:', error);
      setIsSaving(false);
    }
  };
    ...
    <div className="h-full bg-white flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto px-6 pt-8 space-y-8 pb-0 scrollbar-hide">
                ...
                {videoPreviewUrl ? (
                  <div className="relative w-full h-full pointer-events-none">
                    <video 
                      ...
                      onError={() => {
                        console.error('Erro ao carregar preview do vídeo, tentando recuperar...');
                        handleRetry();
                      }}
                    />
                    {isVideoLoading && (
        ...
        <div className="space-y-4">
                ...
                {formData.vehicleImage && formData.vehicleImage.length > 100 && !isProcessingVehicle && (
                  <div className="flex items-center gap-3">
                    <button 
                      onClick={async () => {
                        setIsProcessingVehicle(true);
                        const flipped = await flipImageHorizontal(formData.vehicleImage);
                        setFormData(prev => ({ ...prev, vehicleImage: flipped }));
                        setIsProcessingVehicle(false);
                      }}
          ...
          <div className="space-y-3 pt-4 pb-0">
          ...
          <div className="pt-4 pb-0 flex justify-center">
            <button 
              ...
              className="bg-gray-100 text-black font-bold text-[13px] px-6 py-3 rounded-full uppercase tracking-wider active:scale-95 transition-all shadow-sm border border-gray-200"
            >
              Zerar Taxas
            </button>
          </div>
          ...
          <div className="space-y-4 pt-2">
            <div className="bg-gray-50 rounded-2xl p-4 border border-transparent shadow-sm">
              <p className="text-[10px] font-black text-gray-400 uppercase mb-3 tracking-widest">Dinâmico no Mapa</p>
              <div className="flex gap-2">
                {['Sim', 'Não', 'Sem os preços'].map((option) => (
                  <button
                    key={option}
                    onClick={() => setFormData(prev => ({ ...prev, dynamicOnMap: option }))}
                    className={`flex-1 py-3 rounded-xl font-bold text-[12px] transition-all ${
                      formData.dynamicOnMap === option ? 'bg-black text-white shadow-md' : 'bg-white text-gray-400 border border-gray-100'
                    }`}
                  >
                    {option}
                  </button>
          ...
          {/* Gerenciar Sessões */}
          <div className="space-y-6 pt-6 pb-0 border-t border-gray-100 mt-4">
      ...
      <div className="p-4 pb-12 bg-white border-t border-gray-100 z-[100] shadow-[0_-10px_20px_rgba(0,0,0,0.03)]">
        <motion.button 
          onClick={handleSave}
          ...
          className={`w-full py-5 rounded-2xl text-[20px] font-black shadow-xl active:scale-[0.98] transition-all flex items-center justify-center gap-3 disabled:opacity-70`}
        >
