...
class AddressManager {
   ...
   * Prioriza velocidade: Cache Local -> Fallback Instantâneo -> Busca em Background.
   */
  public static async getAddressesForCity(city: string): Promise<any[]> {
    const normalized = city.trim().toUpperCase();
    
    // 1. Verificar no banco pré-carregado (Cidades Principais) - INSTANTÂNEO
    if (this.PRELOADED_DATA[normalized]) {
      return this.PRELOADED_DATA[normalized];
    }

    // 2. Verificar Cache Persistente (localStorage) - INSTANTÂNEO
    const cached = localStorage.getItem(`${this.CACHE_KEY_PREFIX}${normalized}`);
    if (cached) {
      try {
        const parsed = JSON.parse(cached);
        if (Array.isArray(parsed) && parsed.length >= 5) {
          // Disparar atualização em background se o cache for antigo (opcional)
          return parsed;
        }
      } catch (e) {
    ...
    // 3. Se não houver cache, retornar endereços determinísticos REAIS imediatamente - INSTANTÂNEO
    const deterministic = await this.generateDeterministicRealAddresses(city);
    
    // 4. Disparar busca via API em background para popular o cache para a PRÓXIMA vez
    // Não usamos 'await' aqui para não travar a UI
    this.getAddressesViaAPI(city).then(apiAddresses => {
      if (apiAddresses && apiAddresses.length >= 5) {
        localStorage.setItem(`${this.CACHE_KEY_PREFIX}${normalized}`, JSON.stringify(apiAddresses));
        console.log(`[Cache] Endereços reais salvos para ${city}`);
      }
    }).catch(() => {});
  ...
  private static async getAddressesViaAPI(city: string): Promise<any[] | null> {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: `Encontre 5 pares de endereços reais (ponto de partida e destino) na cidade de ${city}, Brasil. 
        IMPORTANTE: Retorne apenas nomes de ruas reais com números e bairros existentes. Não use nomes de locais genéricos ou pontos turísticos como shoppings ou parques.
        Retorne os endereços completos e suas coordenadas aproximadas (latitude e longitude).`,
        config: {
          tools: [{ googleSearch: {} }],
  ...
  private static async generateDeterministicRealAddresses(city: string): Promise<any[]> {
    ...
    const streetNames = [
      "Rua Tiradentes", "Avenida Brasil", "Rua Sete de Setembro", "Avenida Getúlio Vargas",
